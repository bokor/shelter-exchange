...
describe AccountMailer, ".account_created" do
  ...
  it "contains the correct body" do
    ...
    @email.should have_content("Account Tester")
    @email.should have_content("account_tester@test.com")
  end
end

describe AccountMailer, ".welcome" do

  before do
    @account = Account.gen
    @user = User.gen \
      :account_id => @account.id,
      :name => "JimUser VonTester",
      :email => "account_tester@test.com",
      :role => "owner"
    @shelter = Shelter.gen \
      :account_id => @account.id,
  ...
  it "contains the correct body" do
    ...
    email.should have_content("Hi JimUser,")
    ...
    email.should have_content("Shelter Exchange have partnered with")
    email.should have_link("Petfinder.com", :href => "http://www.petfinder.com")
    email.should have_content(" and ")
    email.should have_link("Adopt-a-pet.com", :href => "http://www.adoptapet.com")
    email.should have_content("to provide simple automatic uploading of animals to their sites. You can implement the automatic uploads by following the directions here:")
    ...
    email.should have_content("Also, once you have signed in to the application you will notice a button called Community.")
    email.should have_content("Here you can view all shelters and rescue groups currently using Shelter Exchange and the animals they have")
    email.should have_content("available. This information is only available to shelters and rescue groups that are using Shelter Exchange, as we")
    email.should have_content("provide an insight into all animals in need, not just those available for adoption. We encourage you to visit this")
    email.should have_content("section often if you can help take in other animals to your facility. This is where we display animals with the")
    email.should have_content("greatest needs or the shortest time to live. Additionally if you have animals that you need to share with other")
    email.should have_content("groups please ensure you enter them in under the correct status. (For example, a hard behavioral case would be")
    email.should have_content("best served as a 'Rescue Candidate.' This informs other rescues that the animal needs assistance and would make")
    email.should have_content("an ideal candidate for a rescue group.)")
    ...
    email.should have_selector(".closing") do |closing|
      ...
      closing.should have_content("Email: ")
