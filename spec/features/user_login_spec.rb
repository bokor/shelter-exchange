...
feature "User Authentication and Login Actions for the application" do
  ...
  background do
    ...
    switch_to_subdomain("login-test")
  end
  ...
  scenario "user forgot their password", :delayed_job => true do
    ...
    expect(current_url).to eq("http://login-test.se.test:9292/login")
