...
run "cd /tmp"
run "exec ssh-agent bash -c 'ssh-add /home/deploy/.ssh/shelter_exchange_app-deploy-key && git clone git://github.com/kmamykin/jammit-s3.git && cd jammit-s3 && git checkout -b remotes/origin/aws-s3'"
# run "rm -rf /tmp/jammit-s3"
run "pwd"
run "gem build jammit-s3.gemspec"
sudo "gem install jammit-s3-0.6.0.2.gem"
