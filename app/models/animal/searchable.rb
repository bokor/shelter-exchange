module Animal::Searchable
  ...
  included do
    ...
    scope :search, lambda { |q| includes(:animal_type, :animal_status, :photos).where("id LIKE ? OR name LIKE ? OR description LIKE ? OR microchip LIKE ? OR
                                                                              color LIKE ? OR weight LIKE ? OR primary_breed LIKE ? OR secondary_breed LIKE ?",
                                                                              "%#{q}%", "%#{q}%", "%#{q}%", "%#{q}%", "%#{q}%", "%#{q}%", "%#{q}%", "%#{q}%") }
    scope :search_by_name, lambda { |q| includes(:animal_type, :animal_status, :photos).where("id LIKE ? OR name LIKE ?", "%#{q}%", "%#{q}%") }                                              
